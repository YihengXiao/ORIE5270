...
def multiply(A, v):
    ...
    ...
    """
    matrix = matrices.map(lambda l: [float(x) for x in l.split(",")]).cache()  # return a RDD
    vector = vectors.map(lambda l: [float(x) for x in l.split(",")]).cache()

    matrix = matrix.map(lambda l: (l[0], [(l[i], i-1) for i in range(1, len(l))]))
    ...
    res = matrix.join(vector).map(lambda l: (l[1][0][0], l[1][0][1]*l[1][1]))
    res = res.reduceByKey(lambda x,y: x+y).sortBy(lambda x: x[0]).map(lambda x: x[1]).collect()
